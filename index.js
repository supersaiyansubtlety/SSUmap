...
function expandNode(d, strict) {
	...
	d3.select('body')
	    ...
	    .text('Collapse Tree')
	    .style('position', 'fixed')
	//.style('left', c.mapWidth + 50)
	//.style('top', c.mapHeight - 50)
...
{
    console.log('Called expandNodeByName');
    nodes.forEach((node) => {
...
{
    console.log('Called expandNodeByNameDelayed');
    window.setTimeout(()=>
		      {
			  setTreeOpacity(1);
